...
import Apaw from "npm:@johnfoderaro/apaw";
...
const apaw = Apaw({
  ...
  region: 'us-west-2',
  key: Deno.env.get('AWS_ACCESS_KEY_ID'),
  secret: Deno.env.get('AWS_SECRET_ACCESS_KEY'),
});

const sleep = (msec: number | undefined) => new Promise(resolve => setTimeout(resolve, msec));

const rakutenEndPoint = `https://app.rakuten.co.jp/services/api/Product/Search/20170426?format=json&applicationId=${Deno.env.get('RAKUTEN_APP_ID')}`;
const getRakutenAffiliateLink = async (eanList: any) => {
  let data = {};
  for (const janCode of eanList) {
...
const getKindleAffiliateLink = async (eanList: any) => {
  let data = {};
  for (const janCode of eanList) {
    console.log(janCode);
...
const yahooEndpoint = `https://shopping.yahooapis.jp/ShoppingWebService/V3/itemSearch?appid=${Deno.env.get('YAHOO_APP_ID')}`;
const getYahooAffiliateLink = async (eanList: any) => {
  let data = {};
  for (const janCode of eanList) {
...
(async () => {
  ...
  if (filename != null && filename != "") {
    ...
    for await (const entry of Deno.readDir(mdDir)) {
  ...
  for (const file of files) {
    console.log(join(mdDir, file));
    const content = await Deno.readTextFile(join(mdDir, file));
  ...
  try {
    ...
    for (let i = 0; i < asin_list.length; i = i + 10) {
      ...
      for (const item of items) {
        ...
        await Deno.writeTextFile(`${dataDir}/${item['ASIN']}.json`, JSON.stringify(item, null, '    '));
      }
