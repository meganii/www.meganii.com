...
lastmod: 2017-11-24T19:22:45+09:00
comments: true
category: ['Tech']
...
「静的」の反対である「動的」なジェネレータとして代表的なものは、[WordPress][]です。[WordPress][]はユーザからのリクエストに応じて毎回ページを動的に生成しています。
...
[HugoのRelated Contentを利用して関連記事を表示する \- SIS Lab](https://www.meganii.com/blog/2017/10/07/hugo-related-content/)
...
1つ目のポイントは「ビルド速度」です。
静的サイトジェネレータは、[Jekyll][] -> [Octopress][] -> [Middleman][]-> [Hugo][]と使ってきましたが、Hugoは圧倒的に生成時間が短いです。
...
[Hugo Discussion](https://discuss.gohugo.io/)では、Hugoを使うときの質問やTipsが日々飛び交っています。また、[spf13/hugo: A Fast and Flexible Static Site Generator built with love in GoLang \[github\]](https://github.com/spf13/hugo)を見てもわかるように、日々新機能の更新、メンテナンスのコミットがあります。この勢いがあれば、使い始めたけれどメンテナンスされなくなったということはないでしょう。
...
静的サイトジェネレータ変更、WordPressから静的サイトジェネレータへの移行を検討されている方は、ぜひHugoも試してみてください！
