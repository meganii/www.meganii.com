...
Hugo 0.28 からPython Pygmentsに頼らず、Goの機能でシンタックスハイライトが可能になりました。これにより、シンタックスハイライトを行なっているコンテンツがあったとしても、キャッシュなしで高速なビルドができるようになっています。
...
`pygmentsUseClasses`を有効にしているので、別途次のコマンドを実行して出てきたcssを自身のcssファイルに書き込みます。
