...
コンテナ内からホスト側のdockerを呼ぶためには、`-v /var/run/docker.sock:/var/run/docker.sock`でホスト側と共有させてあげればよい。
...
{{% quote %}}
1. Mounting /var/run/docker.sock inside the container is necessary to send requests to the Docker remote API.
