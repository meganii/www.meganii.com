...
- `pbpaste`ではImageをペーストできない（Textのみ）
...
`Visual Studio Code`の`Extension`内で完結しないのはいかがなものかと思い、「1.AppleScriptを利用する」で実装することにしました。
...
`filetype`については、[ASH Planning: クリップボード活用のススメ](http://ashplanning.blogspot.com/2006/12/blog-post.html)をご覧ください。
...
`cmd + shift + ctl + 4`でスクリーンショットを取得した後、上記`AppleScript`を`spawn`から実行すると、先ほど取得したスクリーンショットが画像として保存されます。
