...
`Tableau Desktop`の14日トライアルを実施しました。その中で、Google Analyticsを`データソース`として利用したアクセス解析を試してみましたので、内容をまとめます。
...
Google Analyticsのデータを取得するためには必要な権限を付与する必要があります。流れとしては以下の通りです。

### 1. 接続 > サーバへ > その他...から Google アナリティクスを選択
...
接続が成功すれば、紐付くGoogle Analyticsのアカウントが表示されます。これで、Google Analyticsからデータ取得できる準備が整いました。後は、必要に応じて`ディメンション`と`メジャー`を選択します。
...
試行錯誤を行う上で、データ取得に時間が掛かると効率が悪いです。そのため、取得する`ディメンション`と`メジャー`の検討段階においては、日付範囲は１ヶ月など小さくしておき、どのようなデータが取得できるのかを確認しながら作業するのをおすすめします。

ディメンションは7個までという制限があるため、なるべく多くの情報量を持つ`ディメンション`を選択し、Tableau上で形式変換や分割などを行うなどの工夫が必要です。
...
Google Analyticsをデータソースに指定する準備ができました。続いて、データを見ながら思い浮かんだ疑問に対して、いろいろとViz(チャート／グラフ)を作ってみます。
...
「曜日毎のページビュー数」をハイライト表で確認すると、私のブログ(www.megani.com)は、水曜日によく見られる傾向にありました。
...
このことから、企業からの技術カテゴリの記事へのアクセスが多いのではないかと考えました。しかし、水曜日にアクセスが集中するのが理解できません。

そこで、各記事のカテゴリ情報をGoogle Analyticsのデータに`ブレンド`（Google Analyticsのデータソース以外のデータを結合）することで、ブログ記事のカテゴリ毎の傾向を確認してみます。
...
さらに詳しく調べるのであれば、別のデータも必要になってきそうですが、何気なく調べ始めた「曜日毎のページビュー数ってどうなっているか」という疑問から派生して、別の事実が見えてきました。



## Twitterへの投稿は何時がベストか？

続いてTwitterから記事が見られるのはどんな時かを見てみました。

ページビュー数だけを見ると21時がベストという結果でしたが、単純に自分がTwitterに投稿している時間帯と一致するだけなのかもしれないのでそこは追加で調べる必要がありそうです。
...
自分が疑問に思ったことに対して、簡単なドラッグ&ドロップの操作でViz(チャート／グラフ)を作り、疑問を解決できる点については、`Tableau`は非常に優れており、使ってて楽しいツールだと感じました。

Google Analyticsが持つレポーティングツールを利用しても同じようなチャート／グラフを作成することはできるかもしれませんが、やはり自分が見たいデータをドラッグ&ドロップで操作して、描画できるTableauは、より直感的で楽しいです。
