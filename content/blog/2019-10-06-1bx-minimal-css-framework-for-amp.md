...
- 2文字クラス名を多用しているため、HugoのSyntax Highlighting(Chargo)のクラス名と衝突するが、Inlineオプションを利用することで回避可能
...
しかし、AMPの制限で、インライン化するコードは50KB以内に収めなければいけません。
...
こうすることで、CSSにSyntax Highliting用のCSSを書く必要がないので、1BXとクラス名が衝突しなくなります。
